...
@Test func macOSAppIconIsDeclaredGeneratedAndPackaged() throws {
    ...
    #expect(generatorScript.contains("drawWhiteBase()"))
    #expect(generatorScript.contains("drawSubtleRim()"))
    #expect(generatorScript.contains("drawGaugeRing()"))
    #expect(generatorScript.contains("drawGaugeFace()"))
    #expect(generatorScript.contains("drawPulseWaveform()"))
    #expect(generatorScript.contains("drawIconShadow()"))
...
@Test func dashboardStatusRowsKeepLongValuesReadableOnOneLine() throws {
    ...
    let stableMetricText = componentBody(named: "StableMetricText", in: dashboard)

    #expect(statusRow.contains("StableMetricText(text: value, font: .system(size: 13, weight: .semibold), minWidth: DashboardLayout.statValueMinWidth, alignment: .trailing, minimumScaleFactor: 0.78)"))
    #expect(stableMetricText.contains(".lineLimit(1)"))
    #expect(stableMetricText.contains(".minimumScaleFactor(minimumScaleFactor)"))
