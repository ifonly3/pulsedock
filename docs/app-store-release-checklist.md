...
- Repository-local GitHub Pages sources: `docs/support/index.html` and `docs/privacy-policy/index.html`.
- Pre-submit gate: run `scripts/validate-public-pages.sh`; after publishing the docs, run `CHECK_PUBLIC_URLS=1 scripts/validate-public-pages.sh` and require both GitHub Pages URLs to return HTTP 200 before submitting the App Store version.
- External publishing blocker: GitHub Pages still needs to publish the repository-local page sources before App Store submission.
...
Place final English screenshots in `docs/app-store/screenshots/en/`. Keep existing Simplified Chinese screenshots in `docs/app-store/screenshots/zh-Hans/` when shipping Chinese metadata. The flat `docs/app-store/screenshots/` root is structural only and must not contain uploadable `.png`, `.jpg`, or `.jpeg` assets.
...
Validate the existing Simplified Chinese screenshot set when updating `zh-Hans` metadata:
